...
extern "C" {
    ...
    extern const uint8_t _binary_evse_pib_start[];
    extern const uint8_t _binary_evse_pib_end[];
}
...
const embedded_images_t *embedded_images(void)
{
    ...
    img.softloader.size = (uint32_t)(_binary_softloader_nvm_end
                                     - _binary_softloader_nvm_start);
    img.firmware.data   = _binary_firmware_nvm_start;
    img.firmware.size   = (uint32_t)(_binary_firmware_nvm_end
                                     - _binary_firmware_nvm_start);
    img.pib.data        = _binary_evse_pib_start;
    img.pib.size        = (uint32_t)(_binary_evse_pib_end
